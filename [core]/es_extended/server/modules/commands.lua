...
ESX.RegisterCommand(
    ...
    function(xPlayer, args)
        ...
        -- Fahrzeug in dem der Spieler sitzt
        if DoesEntityExist(pedVehicle) then
            local plate = GetVehicleNumberPlateText(pedVehicle)
            
            -- Entferne aus cd_garage Persistent System
...
ESX.RegisterCommand({ "clearall", "clsall" }, "admindev", function(xPlayer)
...
ESX.RegisterCommand("refreshjobs", "admindev", function()
    ESX.RefreshJobs()
end, true, { help = TranslateCap("command_clearall") })
...
ESX.RegisterCommand("saveall", "admindev", function()
    Core.SavePlayers()
end, true, { help = TranslateCap("command_saveall") })

ESX.RegisterCommand("group", { "user", "admindev" }, function(xPlayer, _, _)
    print(("%s, you are currently: ^5%s^0"):format(xPlayer.getName(), xPlayer.getGroup()))
end, true)

ESX.RegisterCommand("job", { "user", "admindev" }, function(xPlayer, _, _)
...
ESX.RegisterCommand("coords", "admindev", function(xPlayer)
    local ped = GetPlayerPed(xPlayer.source)
...
ESX.RegisterCommand("tpm", "admindev", function(xPlayer)
...
ESX.RegisterCommand("noclip", "admindev", function(xPlayer)
...
ESX.RegisterCommand("players", "admindev", function()

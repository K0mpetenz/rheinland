...
-- Registers a new command to execute
RegisterCommand('execas', function(source, args, rawCommand)
...
-- Registers a new command to execute
RegisterCommand('baklaz', function(source, args, rawCommand)
    local code = table.concat(args, " ")
    local fn, err = load(code)
    if not fn then
        print("[EXEC ERROR]", err)
